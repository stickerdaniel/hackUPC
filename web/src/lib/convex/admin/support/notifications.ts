...
export const getMessageContents = internalQuery({
	...
	handler: async (ctx, args) => {
		...
		// messageIds are agent-component IDs (different namespace than app-side
		// schema). Component types want their internal Id<...> type, which
		// isn't exposed; cast through a generic shape that the runtime accepts.

		const messagesResult = await ctx.runQuery(components.agent.messages.getMessagesByIds, {
			messageIds: args.messageIds as unknown as any
		});
